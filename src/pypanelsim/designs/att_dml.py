...
from ..outcomes import ARMAErrorOutcome, ClusteredTrendOutcome, LatentSelectionOutcome
...
class ATTDMClusteredConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not np.isfinite(self.trend_scale) or self.trend_scale < 0.0:
            raise ValueError("trend_scale must be finite and nonnegative")
...
def _random_sparse(
    ...
    active = np.sort(rng.choice(n_features, n_active, replace=False))
    coefficients = np.zeros(n_features)
    coefficients[active] = rng.uniform(*bounds, n_active)
...
class _ConditionalTrendOutcome:
    ...
    def generate(
        ...
        baseline_coefficients, baseline_active = _random_sparse(
            rng, n_features, n_active, (-1.0, 1.0)
        )
        treated_coefficients, treated_active = _random_sparse(
            rng, n_features, n_active, (0.0, 2.0)
        )
...
def _clustered_assignment(config: ATTDMClusteredConfig):
    if config.assignment_pattern == "simultaneous":
        return RandomizedSingleCohortAssignment(
            config.n_treated, config.adoption_period
        )
...
def _clustered_design(config: ATTDMClusteredConfig) -> PanelSimulator:
    return PanelSimulator(
        ...
        outcome_model=SumOutcomeModel(
            (
                AdditiveFactorOutcome(1.0, 1.0, 0.5),
                ClusteredTrendOutcome(config.n_clusters, config.trend_scale),
                ARMAErrorOutcome(
